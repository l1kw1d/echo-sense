...
var util = {
    ...
    fromUSD: function(usd_cents, currency, forexTable) {
        // Returns cents in passed currency
        if (!currency || currency == "USD") return parseInt(usd_cents);
        else {
            var er = forexTable[currency];
            if (er) {
                cents = usd_cents * er.rate;
                return parseInt(cents);
            } else {
                console.error("Couldn't get exchange rate for " + currency);
                return null;
            }
        }
    },

    toUSD: function(cents, currency, forexTable) {
        // Returns cents in USD
        if (!currency || currency == "USD") return parseInt(cents);
        else {
            var er = forexTable[currency];
            if (er) {
                var usd_cents = cents / er.rate;
                return parseInt(usd_cents);
            } else {
                console.error("Couldn't get exchange rate for " + currency);
                return null;
            }
